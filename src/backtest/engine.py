...
def _atr(frame: pd.DataFrame, period: int) -> pd.Series:
    ...
    true_range = pd.concat([frame["high"] - frame["low"], (frame["high"] - prev_close).abs(), (frame["low"] - prev_close).abs()], axis=1).max(axis=1)
...
def run_backtest(frame: pd.DataFrame, strategy: SignalFn, config: BacktestConfig | None = None) -> BacktestResult:
    ...
    def execute(action: Action, index: int, reason: str = "SIGNAL", quantity_override: int | None = None) -> int:
        nonlocal cash, quantity, entry_time, entry_price, entry_fees, entry_slippage_cost, entry_atr, highest_high, hold_bars, peak_stage
        ...
        if action == "BUY" and quantity == 0:
            ...
            cash -= notional + fees
            quantity = max_qty
            entry_time, entry_price = timestamp, execution_price
            entry_fees = fees
            ...
            highest_high, hold_bars, peak_stage = highs[index], 0, 0
            return max_qty
        if action == "SELL" and quantity > 0:
            ...
            trades.append(Trade(code, entry_time or timestamp, entry_price, timestamp, execution_price, sell_qty, gross_pnl, total_fees, allocated_entry_slippage + exit_slippage_cost, net_pnl, return_pct, reason))
            quantity -= sell_qty
            entry_fees -= allocated_entry_fees
            entry_slippage_cost -= allocated_entry_slippage
            if quantity == 0:
                entry_time = None
                entry_price = entry_fees = entry_slippage_cost = 0.0
                entry_atr = None
                highest_high = 0.0
                hold_bars = peak_stage = 0
            return sell_qty
        return 0
    ...
    def risk_exit(index: int) -> bool:
        ...
        if stop_price is not None and low <= stop_price:
            execute("SELL", index, "ATR_STOP"); return True
        if target_price is not None and high >= target_price:
            execute("SELL", index, "ATR_TARGET"); return True
        if config.peak_retrace_2_pct is not None and peak_stage < 2:
            ...
            if retrace_2 >= config.peak_retrace_2_pct and peak_stage >= 1:
                ...
                execute("SELL", index, "PEAK_RETRACE_2", sell_qty); peak_stage = 2
                return quantity == 0
    ...
    for i in range(len(bars)):
        ...
        if pending_index == i:
            execute(pending, i); pending = "HOLD"; pending_index = None
        ...
        if risk_triggered:
            ...
            action = strategy(bars.iloc[i], None)
        ...
        if config.execution == "close":
            ...
            pending, pending_index = action, i + 1
        elif config.force_close_eod:
            if quantity > 0:
                execute("SELL", i, "EOD")
            pending, pending_index = "HOLD", None
        mark = cash + quantity * closes[i]
        equity_rows.append({"datetime": pd.Timestamp(datetimes[i]), "equity": cash if is_day_end and quantity == 0 else mark})
    ...
    return BacktestResult(code, config.initial_cash, cash, (cash / config.initial_cash - 1.0) * 100.0, max_dd, len(trades), wins, losses, wins / len(trades) * 100.0 if trades else 0.0, profit_factor, trades, equity)
