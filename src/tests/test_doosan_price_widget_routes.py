...
from src.web import doosan_price_widget_routes as routes


def _client(monkeypatch):
    monkeypatch.setenv("KORSTOCKSCAN_DOOSAN_WIDGET_ACCESS_KEY", "secret")
    monkeypatch.setenv(
        "KORSTOCKSCAN_DOOSAN_WIDGET_SNAPSHOT_PATH", "/tmp/no-doosan-snapshot.json"
    )
    app = Flask(__name__)
    app.register_blueprint(routes.doosan_price_widget_bp)
    return app.test_client()


def test_route_rejects_wrong_access_key(monkeypatch):
    client = _client(monkeypatch)
    assert client.get("/api/widget/doosan-price").status_code == 401
    assert (
        client.get(
            "/api/widget/doosan-price",
...
def test_route_uses_cached_token_and_quote_only_fallback(monkeypatch):
    ...
    assert captured == [("/api/dostk/stkinfo", "ka10001", {"stk_cd": "034020"})]


def test_doosan_dedicated_key_file_precedes_shared_direct_key(monkeypatch, tmp_path):
    key_file = tmp_path / "doosan.key"
    key_file.write_text("dedicated-file-key\n", encoding="utf-8")
    monkeypatch.delenv("KORSTOCKSCAN_DOOSAN_WIDGET_ACCESS_KEY", raising=False)
    monkeypatch.setenv("KORSTOCKSCAN_DOOSAN_WIDGET_ACCESS_KEY_FILE", str(key_file))
