# Key Lineage Ledger - 2026-07-13

## Decision
- source keys: `298`
- runtime observation target date: `2026-07-13`
- runtime policy source date: `2026-07-10`
- postclose candidate source date: `2026-07-13`
...
- active sim policy windows: events=`594` zero_count=`594` positive_count=`0` id_without_count=`0` loaded_for_effect=`False` zero_count_effect_excluded=`True`
- active sim taxonomy contracts: pending=`35` counts=`{'canonical': 93, 'new_axis_pending_taxonomy': 35, 'unknown': 16}`
- event IO guard: `{'mode': 'streaming_jsonl', 'untracked_value_limit_per_field': 200000, 'line_bytes_limit': 8000000, 'files_seen': 1, 'lines_read': 114448, 'json_decode_error_count': 0, 'file_read_error_count': 0, 'oversized_line_skipped_count': 0, 'truncated_untracked_value_count': 0, 'truncated_untracked_value_count_by_field': {}, 'truncated_panic_sim_record_id_count': 0, 'truncated_panic_no_match_sim_record_id_count': 0}`
- active seed candidate validation: total=`594` eligible=`594` not_match_eligible=`0` not_match_eligible_reasons=`{}` new_entry=`15` followup=`579` matched=`0` matched_true_without_seed_id=`0` unmatched=`594` new_entry_unmatched=`15` followup_unmatched=`579` eligible_without_seed_id=`594` without_seed_details=`{'parent_seed_id_not_propagated_to_followup': 579, 'taxonomy_pending_or_natural_no_match': 15}` inferred_parent_seed_id=`0` inferred_stages=`{}` ambiguous_prefix=`0` missing_parent_stages=`{'scalp_sim_entry_ai_price_applied': 13, 'scalp_sim_pre_submit_liquidity_guard_would_pass': 10, 'scalp_sim_pre_submit_overbought_guard_would_pass': 15, 'scalp_sim_buy_order_virtual_pending': 15, 'scalp_sim_buy_order_assumed_filled': 15, 'scalp_sim_holding_started': 15, 'scalp_sim_sell_order_assumed_filled': 9, 'scalp_sim_panic_level1_entry_observed': 14, 'scalp_sim_panic_scale_in_blocked': 468, 'scalp_sim_pre_submit_liquidity_guard_would_block': 5}` raw_without_seed_id=`594` eligible_followup_without_seed_id=`579` raw_followup_without_seed_id=`579`
...
- blockers: mismatch=`0`, catalog_missing=`0`, preopen_missing=`168`, not_instrumented=`0`
