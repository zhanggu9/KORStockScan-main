# Key Lineage Ledger - 2026-07-01

## Decision
- source keys: `282`
- runtime observation target date: `2026-07-01`
- runtime policy source date: `2026-06-30`
- postclose candidate source date: `2026-07-01`
...
- active sim policy windows: events=`1549` zero_count=`0` positive_count=`1549` id_without_count=`0` loaded_for_effect=`True` zero_count_effect_excluded=`False`
- active sim taxonomy contracts: pending=`15` counts=`{'canonical': 95, 'new_axis_pending_taxonomy': 15, 'unknown': 16}`
- event IO guard: `{'mode': 'streaming_jsonl', 'untracked_value_limit_per_field': 200000, 'line_bytes_limit': 8000000, 'files_seen': 1, 'lines_read': 156472, 'json_decode_error_count': 0, 'file_read_error_count': 0, 'oversized_line_skipped_count': 0, 'truncated_untracked_value_count': 0, 'truncated_untracked_value_count_by_field': {}, 'truncated_panic_sim_record_id_count': 0, 'truncated_panic_no_match_sim_record_id_count': 0}`
- active seed candidate validation: total=`1346` eligible=`1346` not_match_eligible=`0` not_match_eligible_reasons=`{}` new_entry=`61` followup=`1285` matched=`921` matched_true_without_seed_id=`0` unmatched=`425` new_entry_unmatched=`20` followup_unmatched=`405` eligible_without_seed_id=`425` without_seed_details=`{'parent_seed_id_not_propagated_to_followup': 405, 'taxonomy_pending_or_natural_no_match': 20}` inferred_parent_seed_id=`0` inferred_stages=`{}` ambiguous_prefix=`0` missing_parent_stages=`{'scalp_sim_panic_level1_entry_observed': 12, 'scalp_sim_pre_submit_liquidity_guard_would_pass': 18, 'scalp_sim_pre_submit_overbought_guard_would_pass': 20, 'scalp_sim_buy_order_virtual_pending': 20, 'scalp_sim_buy_order_assumed_filled': 20, 'scalp_sim_holding_started': 20, 'scalp_sim_panic_scale_in_blocked': 275, 'scalp_sim_sell_order_assumed_filled': 4, 'scalp_sim_entry_ai_price_applied': 10, 'scalp_sim_panic_bottoming_entry_allowed': 3, 'scalp_sim_pre_submit_liquidity_guard_would_block': 2, 'scalp_sim_scale_in_order_unfilled': 1}` raw_without_seed_id=`425` eligible_followup_without_seed_id=`405` raw_followup_without_seed_id=`405`
- panic scale-in no-match: events=`1028` unique_sim_records=`45` missing_sim_record_id=`0` repeated_followup=`983` status_counts=`{'no_match': 1028}` source_stage_counts=`{'first_ai_wait': 467, 'blocked_ai_score': 410, 'scale_in': 151}`
- blockers: mismatch=`0`, catalog_missing=`0`, preopen_missing=`139`, not_instrumented=`0`
