# Swing Lifecycle Audit - 2026-07-03
...
- panic_state: `NORMAL`
- panic_active_sim_probe: `{'active_positions': 7, 'profit_sample': 4, 'avg_unrealized_profit_rate_pct': -0.4295, 'win_rate_pct': 0.0, 'wins': 0, 'losses': 4, 'flat': 0}`
- panic_origin_outcome: `{'blocked_gatekeeper_reject': {'count': 1, 'avg_profit_rate_pct': None}, 'blocked_swing_score_vpw': {'count': 1, 'avg_profit_rate_pct': None}, 'market_regime_prior_observed': {'count': 1, 'avg_profit_rate_pct': None}, 'unknown': {'count': 4, 'avg_profit_rate_pct': -0.4295}}`
...
| --- | ---: | ---: |
| `holding_flow_ofi_smoothing_applied` | 20 | 9 |
| `sell_order_sent` | 20 | 20 |
...
- stale_missing_reason_counts: `{'micro_missing': 20, 'micro_not_ready': 9, 'state_insufficient': 9}`
- stale_missing_reason_combination_counts: `{'micro_missing': 11, 'micro_missing+micro_not_ready+state_insufficient': 9}`
- stale_missing_reason_combination_unique_record_counts: `{'micro_missing': 5, 'micro_missing+micro_not_ready+state_insufficient': 4}`
- stale_missing_reason_counts_by_group: `{'exit': {'micro_missing': 20, 'micro_not_ready': 9, 'state_insufficient': 9}}`
- stale_missing_reason_unique_record_counts_by_group: `{'exit': {'micro_missing': 7, 'micro_not_ready': 4, 'state_insufficient': 4}}`
- stale_missing_group_counts: `{'exit': 20}`
...
- orderbook_micro_reason_counts_by_group: `{'exit': {'ready': 11, 'insufficient_samples': 9}}`
- observer_missing_reason_counts_by_group: `{'exit': {'ok': 20}}`
- source_quality_status_counts_by_group: `{'exit': {'UNKNOWN': 20}}`
- ws_quote_source_counts_by_group: `{'exit': {'UNKNOWN': 20}}`
- ws_quote_stale_counts_by_group: `{'exit': {'UNKNOWN': 20}}`
- entry_micro_state_counts: `{}`
- scale_in_micro_state_counts: `{}`
- exit_micro_state_counts: `{'neutral': 11, 'insufficient': 9}`
- exit_smoothing_action_counts: `{'NO_CHANGE': 20}`
...
| `swing_holding_mfe_mae_defer` | `holding` | `swing_holding_flow_defer` | 9 | `instrumentation_gap` |
| `swing_scale_in_avg_down_pyramid` | `scale_in` | `swing_pyramid_trigger` | 0 | `hold_sample` |
| `swing_exit_post_sell_attribution` | `exit` | `swing_trailing_stop_time_stop` | 20 | `ready` |
...
| `swing_exit_ofi_qi_smoothing` | `holding_exit` | `swing_exit_ofi_qi_smoothing` | 20 | `ready` |
