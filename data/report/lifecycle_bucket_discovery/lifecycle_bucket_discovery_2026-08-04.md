# Lifecycle Bucket Discovery - 2026-08-04

## 판정
- status: `pass`
- source_contract_status: `warning` / changes: `10`
- ai_two_pass_review: `parsed` / model: `sharded` / tier: `tier2`
- ai_review_shards: `3` / `5` parsed, reviewed_candidates=`3`
- surfaced_candidate_count: `62`
- canonical/legacy buckets: `148` / `390`
- dual_proposals: deterministic=`406` ai=`3` hybrid_selected=`3`
- absorbed/source_quality_blocker: `272` / `0`
- lifecycle_flow_parent_granularity: `too_broad` level=`L2_default` parents=`23` target=`30-60`
- lifecycle_flow_absorbed_children: child=`52` sample=`1403` conflict_parents=`0`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`33900`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`51` context_chars=`38940`
- `sim_policy_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`33870`
- `gap_workorder_review` status=`parsed` candidates=`1` omitted=`9` context_chars=`37433`
- `taxonomy_discovery_review` status=`parsed` candidates=`1` omitted=`8` context_chars=`37668`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_avg_down` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`870` ev=`-0.5571` ai_final=`keep` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_pyramid_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`102` ev=`0.3877` ai_final=`-` taxonomy=`absorb_as_dimension`
