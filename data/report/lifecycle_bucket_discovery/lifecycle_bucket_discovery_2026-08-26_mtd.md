# Lifecycle Bucket Discovery - 2026-08-26_mtd
...
- lifecycle_flow_absorbed_children: child=`227` sample=`24291` conflict_parents=`4`
...
- sim_policy_approved_total_count: `22` (direct=`2`, entry_only=`4`, lifecycle_flow=`17`)
- lifecycle_flow_sim_probe_candidate_count: `17`
- source_dimension_gap_count: `112` / actionable_unknown_gap_count: `0`
- quiet_gap_count: `359` / sim_live_connected: `12`
...
- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_unobserved|submit_q` state=`resolution_complete` tag=`resolution_complete` ev_before=`-1.287827` ev_after=`-1.287827` children=`10` sq_gap=`0` strategy_reversal=`0` exclude=`0` collecting=`0` positive_thin=`3` sim_eligible=`False` live_blockers=`['parent_ev_not_positive']`
- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_unobserved|submit_q` state=`resolution_blocked_thin_sample` tag=`sample 부족 keep collecting` ev_before=`-0.467714` ev_after=`-0.467714` children=`7` sq_gap=`0` strategy_reversal=`0` exclude=`0` collecting=`0` positive_thin=`5` sim_eligible=`False` live_blockers=`['parent_ev_not_positive', 'sample_below_live_floor']`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`39969`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`199` context_chars=`46537`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`5` context_chars=`46520`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`39945`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`39941`

### Source Dimension Gap Enrichment
- gap_count: `112` / actionable_unknown_gap_count: `0`
