# Lifecycle Bucket Discovery - 2026-08-12
...
- surfaced_candidate_count: `20`
- canonical/legacy buckets: `114` / `283`
- dual_proposals: deterministic=`292` ai=`1` hybrid_selected=`1`
- absorbed/source_quality_blocker: `190` / `0`
- lifecycle_flow_parent_granularity: `too_broad` level=`L2_default` parents=`12` target=`30-60`
- lifecycle_flow_absorbed_children: child=`20` sample=`958` conflict_parents=`0`
...
- quiet_gap_count: `254` / sim_live_connected: `0`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`31970`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`19` context_chars=`37013`
- `sim_policy_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`31940`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`31946`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`31942`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_avg_down` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`767` ev=`-0.8564` ai_final=`keep` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_pyramid_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`6` ev=`0.218` ai_final=`-` taxonomy=`absorb_as_dimension`
