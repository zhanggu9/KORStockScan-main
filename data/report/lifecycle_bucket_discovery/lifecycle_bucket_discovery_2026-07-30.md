# Lifecycle Bucket Discovery - 2026-07-30
...
- canonical/legacy buckets: `144` / `165`
- dual_proposals: deterministic=`185` ai=`4` hybrid_selected=`2`
- absorbed/source_quality_blocker: `49` / `0`
- lifecycle_flow_parent_granularity: `too_broad` level=`L2_default` parents=`13` target=`30-60`
- lifecycle_flow_absorbed_children: child=`15` sample=`92` conflict_parents=`0`
...
- source_dimension_gap_count: `15` / actionable_unknown_gap_count: `0`
- quiet_gap_count: `138` / sim_live_connected: `0`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`35385`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`14` context_chars=`40423`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`0` context_chars=`38878`
- `gap_workorder_review` status=`parsed` candidates=`1` omitted=`11` context_chars=`38919`
- `taxonomy_discovery_review` status=`parsed` candidates=`1` omitted=`10` context_chars=`39102`

### Source Dimension Gap Enrichment
- gap_count: `15` / actionable_unknown_gap_count: `0`
- join_gap_candidate_count: `2` / sampled: `2`
- join_gap_stage_counts: `{'entry': 2}`
- join_gap_bucket_type_counts: `{'combo_entry_spot': 1, 'score_band': 1}`
- join_gap_recommended_next_action: `enrich_bucket_label_or_join_key_before_bucket_decision`

- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_avg_down` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`51` ev=`-0.7548` ai_final=`keep` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_pyramid_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`20` ev=`0.3976` ai_final=`-` taxonomy=`absorb_as_dimension`
...
- `scale_in:stage_policy:scale_in_weighted_adm_v1` stage=`scale_in` state=`sim_auto_approved` action=`NO_CHANGE` relation=`existing_bucket_refinement` canonical=`scale_in:stage_policy:scale_in:weighted_adm_v1` joined=`71` ev=`-0.4302` ai_final=`keep` taxonomy=`keep_bucket`
