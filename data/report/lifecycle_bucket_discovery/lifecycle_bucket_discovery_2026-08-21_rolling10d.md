# Lifecycle Bucket Discovery - 2026-08-21_rolling10d
...
- canonical/legacy buckets: `116` / `492`
- dual_proposals: deterministic=`500` ai=`68` hybrid_selected=`68`
- absorbed/source_quality_blocker: `393` / `0`
- lifecycle_flow_parent_granularity: `target_pass` level=`L2_default` parents=`51` target=`30-60`
- lifecycle_flow_absorbed_children: child=`133` sample=`12524` conflict_parents=`3`
- ldm_refinement_pressure: input=`3` consumed=`3` closures=`{'new_parent_candidate_created': 2, 'rare_observation_only_budget_capped': 1}`
- active_sim_priority: eligible=`2` active=`2` source_blocked=`11` parent_count_decoupled=`0`
...
- quiet_gap_count: `387` / sim_live_connected: `4`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`39153`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`132` context_chars=`45721`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`1` context_chars=`45683`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`39129`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`39125`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_63_65_source_scalp_sim_entry_ai_price_skip_order_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=score_watch_recovery|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`3` ev=`-0.8214` ai_final=`-` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_lt60_source_scalp_entry_action_decision_snapshot_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`3` ev=`-0.9233` ai_final=`-` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_combo_holding_flow_source_scalp_sim_ov` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`3` ev=`-1.7675` ai_final=`-` taxonomy=`absorb_as_dimension`
