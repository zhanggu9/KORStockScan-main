# Lifecycle Bucket Discovery - 2026-08-07
...
- canonical/legacy buckets: `114` / `239`
- dual_proposals: deterministic=`246` ai=`1` hybrid_selected=`1`
- absorbed/source_quality_blocker: `142` / `0`
- lifecycle_flow_parent_granularity: `too_broad` level=`L2_default` parents=`13` target=`30-60`
- lifecycle_flow_absorbed_children: child=`26` sample=`370` conflict_parents=`1`
- ldm_refinement_pressure: input=`3` consumed=`3` closures=`{'new_parent_candidate_created': 3}`
- active_sim_priority: eligible=`0` active=`3` source_blocked=`11` parent_count_decoupled=`0`
...
- quiet_gap_count: `207` / sim_live_connected: `0`
...
- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_unobserved|entry_so` state=`resolution_blocked_source_quality` tag=`source-quality 때문에 판정 불가` ev_before=`-1.52537` ev_after=`-1.52537` children=`2` sq_gap=`2` strategy_reversal=`0` exclude=`0` collecting=`0` positive_thin=`0` sim_eligible=`False` live_blockers=`['source_quality_gap_children', 'parent_ev_not_positive']`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`32133`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`25` context_chars=`37173`
- `sim_policy_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`32103`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`32109`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`32105`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_avg_down` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`138` ev=`-1.6646` ai_final=`keep` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_pyramid_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`10` ev=`0.396` ai_final=`-` taxonomy=`absorb_as_dimension`
