# Lifecycle Bucket Discovery - 2026-08-31_mtd
...
- dual_proposals: deterministic=`500` ai=`129` hybrid_selected=`129`
...
- lifecycle_flow_absorbed_children: child=`252` sample=`27707` conflict_parents=`4`
- ldm_refinement_pressure: input=`2` consumed=`2` closures=`{'new_parent_candidate_created': 2}`
- active_sim_priority: eligible=`3` active=`3` source_blocked=`5` parent_count_decoupled=`0`
...
- source_dimension_gap_count: `118` / actionable_unknown_gap_count: `0`
- quiet_gap_count: `352` / sim_live_connected: `15`
...
- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_unobserved|submit_q` state=`resolution_complete` tag=`resolution_complete` ev_before=`-1.345044` ev_after=`-1.345044` children=`15` sq_gap=`0` strategy_reversal=`0` exclude=`0` collecting=`0` positive_thin=`3` sim_eligible=`False` live_blockers=`['parent_ev_not_positive']`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`40553`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`199` context_chars=`47102`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`6` context_chars=`47104`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`40529`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`40525`

### Source Dimension Gap Enrichment
- gap_count: `118` / actionable_unknown_gap_count: `0`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_lt60_source_scalp_entry_action_decision_snapshot_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`9` ev=`-0.9422` ai_final=`-` taxonomy=`absorb_as_dimension`
