# Lifecycle Bucket Discovery - 2026-07-14_rolling10d
...
- canonical/legacy buckets: `59` / `500`
- dual_proposals: deterministic=`500` ai=`49` hybrid_selected=`49`
- absorbed/source_quality_blocker: `450` / `0`
- lifecycle_flow_parent_granularity: `target_pass` level=`L1_broad` parents=`31` target=`30-60`
- lifecycle_flow_absorbed_children: child=`202` sample=`12761` conflict_parents=`10`
...
- source_dimension_gap_count: `137` / actionable_unknown_gap_count: `0`
- quiet_gap_count: `370` / sim_live_connected: `11`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`39721`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`199` context_chars=`46289`
- `sim_policy_review` status=`parse_rejected` candidates=`1` omitted=`8` context_chars=`46272`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`39697`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`39693`

### Source Dimension Gap Enrichment
- gap_count: `137` / actionable_unknown_gap_count: `0`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_avg_down` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`104` ev=`-1.0536` ai_final=`-` taxonomy=`absorb_as_dimension`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_none_exit_ex` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`9` ev=`-0.8344` ai_final=`-` taxonomy=`absorb_as_dimension`
