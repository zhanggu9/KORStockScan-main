# Lifecycle Bucket Discovery - 2026-07-31_rolling5d
...
- canonical/legacy buckets: `180` / `426`
- dual_proposals: deterministic=`456` ai=`22` hybrid_selected=`22`
- absorbed/source_quality_blocker: `288` / `0`
- lifecycle_flow_parent_granularity: `too_broad` level=`L3_detailed` parents=`29` target=`30-60`
- lifecycle_flow_absorbed_children: child=`64` sample=`1067` conflict_parents=`0`
...
- quiet_gap_count: `354` / sim_live_connected: `0`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`33408`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`63` context_chars=`39973`
- `sim_policy_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`33378`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`33384`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`33380`

### Source Dimension Gap Enrichment
- gap_count: `69` / actionable_unknown_gap_count: `0`
- join_gap_candidate_count: `7` / sampled: `7`
- join_gap_stage_counts: `{'entry': 6, 'scale_in': 1}`
- join_gap_bucket_type_counts: `{'combo_entry_spot': 4, 'score_band': 1, 'strength_bucket': 1, 'ai_score_band': 1}`
- join_gap_recommended_next_action: `enrich_bucket_label_or_join_key_before_bucket_decision`

- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_avg_down` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`476` ev=`-1.0056` ai_final=`-` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_pyramid_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`relax_or_recover` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`89` ev=`0.3926` ai_final=`-` taxonomy=`absorb_as_dimension`
