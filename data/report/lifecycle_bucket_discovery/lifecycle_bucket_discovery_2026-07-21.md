# Lifecycle Bucket Discovery - 2026-07-21
...
- canonical/legacy buckets: `130` / `179`
- dual_proposals: deterministic=`194` ai=`4` hybrid_selected=`3`
- absorbed/source_quality_blocker: `72` / `0`
- lifecycle_flow_parent_granularity: `too_broad` level=`L2_default` parents=`13` target=`30-60`
- lifecycle_flow_absorbed_children: child=`24` sample=`160` conflict_parents=`0`
...
- quiet_gap_count: `133` / sim_live_connected: `0`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`35386`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`23` context_chars=`40424`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`0` context_chars=`38879`
- `gap_workorder_review` status=`parsed` candidates=`1` omitted=`11` context_chars=`38920`
- `taxonomy_discovery_review` status=`parsed` candidates=`1` omitted=`10` context_chars=`39103`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_avg_down` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`39` ev=`-0.5935` ai_final=`keep` taxonomy=`absorb_as_dimension`
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_missing_scale_in_scale_in_arm_pyramid_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`keep_collecting` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`7` ev=`1.2543` ai_final=`-` taxonomy=`absorb_as_dimension`
