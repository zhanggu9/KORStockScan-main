# Lifecycle Bucket Discovery - 2026-08-05
...
- surfaced_candidate_count: `25`
- canonical/legacy buckets: `123` / `266`
- dual_proposals: deterministic=`278` ai=`1` hybrid_selected=`0`
- absorbed/source_quality_blocker: `164` / `0`
- lifecycle_flow_parent_granularity: `too_broad` level=`L2_default` parents=`11` target=`30-60`
- lifecycle_flow_absorbed_children: child=`25` sample=`382` conflict_parents=`1`
- ldm_refinement_pressure: input=`4` consumed=`4` closures=`{'new_parent_candidate_created': 2, 'rare_observation_only_budget_capped': 2}`
- active_sim_priority: eligible=`0` active=`3` source_blocked=`8` parent_count_decoupled=`0`
...
- quiet_gap_count: `247` / sim_live_connected: `0`
...
- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_unobserved|entry_so` state=`resolution_blocked_source_quality` tag=`source-quality 때문에 판정 불가` ev_before=`-1.003726` ev_after=`-1.003726` children=`2` sq_gap=`2` strategy_reversal=`0` exclude=`0` collecting=`0` positive_thin=`0` sim_eligible=`False` live_blockers=`['source_quality_gap_children', 'parent_ev_not_positive']`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`32160`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`24` context_chars=`37198`
- `sim_policy_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`32130`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`32136`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`32132`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_63_65_source_scalp_sim_entry_ai_price_skip_order_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=score_watch_recovery|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`1` ev=`-0.56` ai_final=`-` taxonomy=`absorb_as_dimension`
