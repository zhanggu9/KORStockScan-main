# Lifecycle Bucket Discovery - 2026-08-19_rolling10d
...
- canonical/legacy buckets: `109` / `493`
- dual_proposals: deterministic=`500` ai=`59` hybrid_selected=`59`
...
- lifecycle_flow_absorbed_children: child=`122` sample=`9975` conflict_parents=`4`
...
- quiet_gap_count: `395` / sim_live_connected: `4`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`37249`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`121` context_chars=`43817`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`1` context_chars=`43779`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`37225`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`37221`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_missing_submit_submit_missing_holding_holding_combo_holding_flow_source_scalp_sim_ov` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`2` ev=`-0.3037` ai_final=`-` taxonomy=`absorb_as_dimension`
