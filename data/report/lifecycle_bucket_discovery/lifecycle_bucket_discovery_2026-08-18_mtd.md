# Lifecycle Bucket Discovery - 2026-08-18_mtd
...
- surfaced_candidate_count: `158`
- canonical/legacy buckets: `43` / `500`
- dual_proposals: deterministic=`500` ai=`57` hybrid_selected=`57`
...
- lifecycle_flow_absorbed_children: child=`151` sample=`13093` conflict_parents=`4`
...
- source_dimension_gap_count: `97` / actionable_unknown_gap_count: `0`
...
- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_unobserved|entry_so` state=`resolution_blocked_thin_sample` tag=`sample 부족 keep collecting` ev_before=`-1.018` ev_after=`-1.018` children=`5` sq_gap=`0` strategy_reversal=`0` exclude=`0` collecting=`0` positive_thin=`0` sim_eligible=`False` live_blockers=`['parent_ev_not_positive', 'sample_below_live_floor']`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`41084`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`150` context_chars=`47652`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`9` context_chars=`44584`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`41060`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`41056`

### Source Dimension Gap Enrichment
- gap_count: `97` / actionable_unknown_gap_count: `0`
