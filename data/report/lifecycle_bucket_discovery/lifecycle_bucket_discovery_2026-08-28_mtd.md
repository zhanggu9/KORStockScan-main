# Lifecycle Bucket Discovery - 2026-08-28_mtd
...
- surfaced_candidate_count: `262`
- canonical/legacy buckets: `45` / `500`
- dual_proposals: deterministic=`500` ai=`125` hybrid_selected=`107`
...
- lifecycle_flow_absorbed_children: child=`245` sample=`26412` conflict_parents=`4`
- ldm_refinement_pressure: input=`3` consumed=`3` closures=`{'new_parent_candidate_created': 2, 'rare_observation_only_budget_capped': 1}`
- active_sim_priority: eligible=`3` active=`4` source_blocked=`5` parent_count_decoupled=`0`
...
- sim_policy_approved_total_count: `26` (direct=`1`, entry_only=`4`, lifecycle_flow=`21`)
- lifecycle_flow_sim_probe_candidate_count: `21`
- source_dimension_gap_count: `116` / actionable_unknown_gap_count: `0`
- quiet_gap_count: `354` / sim_live_connected: `15`
- live_auto_apply_ready_count: `0`
- human_intervention_required: `False`
- warnings: `[]`

## 판정 (Conflict Resolution)
- parent_conflict_resolution_count: `4`
- sim_eligible_after_resolution: `0`
- resolution_states: `{'resolution_complete': 2, 'resolution_blocked_thin_sample': 2}`

- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_unobserved|submit_q` state=`resolution_complete` tag=`resolution_complete` ev_before=`-1.38564` ev_after=`-1.38564` children=`14` sq_gap=`0` strategy_reversal=`0` exclude=`0` collecting=`0` positive_thin=`3` sim_eligible=`False` live_blockers=`['parent_ev_not_positive']`
- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_unobserved|submit_q` state=`resolution_complete` tag=`resolution_complete` ev_before=`-0.46748` ev_after=`-0.46748` children=`9` sq_gap=`0` strategy_reversal=`0` exclude=`0` collecting=`0` positive_thin=`6` sim_eligible=`False` live_blockers=`['parent_ev_not_positive']`
- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_unobserved|submit_q` state=`resolution_blocked_thin_sample` tag=`sample 부족 keep collecting` ev_before=`0.1848` ev_after=`0.1848` children=`6` sq_gap=`0` strategy_reversal=`0` exclude=`0` collecting=`2` positive_thin=`0` sim_eligible=`False` live_blockers=`['sample_below_live_floor']`
- conflict_parent=`lifecycle_flow:combo_lifecycle_flow:entry_score_parent=score_watch_recovery|subm` state=`resolution_blocked_thin_sample` tag=`sample 부족 keep collecting` ev_before=`-0.76956` ev_after=`-0.76956` children=`5` sq_gap=`0` strategy_reversal=`0` exclude=`0` collecting=`0` positive_thin=`2` sim_eligible=`False` live_blockers=`['parent_ev_not_positive', 'sample_below_live_floor']`
...
### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`40595`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`199` context_chars=`47144`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`6` context_chars=`47146`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`40571`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`40567`

### Source Dimension Gap Enrichment
- gap_count: `116` / actionable_unknown_gap_count: `0`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_lt60_source_scalp_entry_action_decision_snapshot_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`7` ev=`-0.9329` ai_final=`-` taxonomy=`absorb_as_dimension`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_lt60_source_scalp_entry_action_decision_snapshot_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`3` ev=`-1.9` ai_final=`-` taxonomy=`absorb_as_dimension`
