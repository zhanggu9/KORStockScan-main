# Lifecycle Bucket Discovery - 2026-08-05_rolling10d
...
- surfaced_candidate_count: `104`
- canonical/legacy buckets: `100` / `495`
- dual_proposals: deterministic=`500` ai=`12` hybrid_selected=`12`
- absorbed/source_quality_blocker: `409` / `0`
- lifecycle_flow_parent_granularity: `target_pass` level=`L2_default` parents=`38` target=`30-60`
- lifecycle_flow_absorbed_children: child=`101` sample=`3617` conflict_parents=`1`
...
- quiet_gap_count: `384` / sim_live_connected: `1`
...
- audit_status: `pass`
- audit_issues: `['Current embedded AI Tier2 proposal is a placeholder reject/not_provided and should not be treated as a substantive taxonomy review.', 'Deterministic-vs-AI comparison was not previously populated for this surfaced candidate.']`
- audit_reason: `sharded review aggregate`

### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`36548`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`100` context_chars=`43126`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`2` context_chars=`40047`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`36524`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`36520`

### Source Dimension Gap Enrichment
- gap_count: `91` / actionable_unknown_gap_count: `0`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_63_65_source_scalp_sim_entry_ai_price_skip_order_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=score_watch_recovery|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`3` ev=`-0.93` ai_final=`-` taxonomy=`absorb_as_dimension`
...
- `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_lt60_source_scalp_entry_action_decision_snapshot_stale_` stage=`lifecycle_flow` state=`source_only_keep_collecting` action=`tighten_or_exclude` relation=`existing_bucket_refinement` canonical=`lifecycle_flow:combo_lifecycle_flow:entry=entry_observed|submit=submit_observed|holding=holding_observed|scale_in=scale_in_observed|exit=exit_observed` joined=`3` ev=`-0.4433` ai_final=`-` taxonomy=`absorb_as_dimension`
