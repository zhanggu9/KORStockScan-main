# Lifecycle Bucket Discovery - 2026-08-04_mtd
...
- canonical/legacy buckets: `139` / `483`
- dual_proposals: deterministic=`500` ai=`8` hybrid_selected=`8`
- absorbed/source_quality_blocker: `375` / `0`
- lifecycle_flow_parent_granularity: `too_broad` level=`L2_default` parents=`26` target=`30-60`
- lifecycle_flow_absorbed_children: child=`64` sample=`2134` conflict_parents=`0`
...
- source_dimension_gap_count: `57` / actionable_unknown_gap_count: `0`
- quiet_gap_count: `416` / sim_live_connected: `0`
...
- audit_status: `pass`
- audit_issues: `['interpreted_state corrected from lifecycle_flow_sim_probe_candidate to allowed schema state sim_auto_approved for review output normalization', 'candidate is not eligible for live_auto_apply_ready because no live_auto_apply_family and deterministic state is probe/handoff only', 'current_ai_tier2_proposal in source was unavailable/reject placeholder, so independent Tier2 proposal supplied']`
- audit_reason: `sharded review aggregate`

### AI Review Shards
- `live_contract_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`36505`
- `lifecycle_flow_review` status=`parsed` candidates=`1` omitted=`63` context_chars=`43082`
- `sim_policy_review` status=`parsed` candidates=`1` omitted=`6` context_chars=`40005`
- `gap_workorder_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`36481`
- `taxonomy_discovery_review` status=`skipped_empty` candidates=`0` omitted=`0` context_chars=`36477`

### Source Dimension Gap Enrichment
- gap_count: `57` / actionable_unknown_gap_count: `0`
