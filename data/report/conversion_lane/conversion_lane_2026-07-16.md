...
- positive EV sample-floor provenance: scope=`conversion_candidates` window=`source_report_window` window_counts=`{'source_report_window': 89}` basis=`candidate_sample_vs_required_sample`
- active sim policy windows: events=`688` zero_count=`688` positive_count=`0` id_without_count=`0` zero_count_effect_excluded=`True`
- active sim taxonomy contracts: pending=`40` counts=`{'canonical': 107, 'new_axis_pending_taxonomy': 40, 'unknown': 13}`
- active seed candidate validation: total=`680` eligible=`0` not_match_eligible=`680` not_match_eligible_reasons=`{'policy_active_seed_count_zero_effect_excluded': 680}` new_entry=`7` followup=`673` matched=`0` matched_true_without_seed_id=`0` unmatched=`0` new_entry_unmatched=`0` followup_unmatched=`0` eligible_without_seed_id=`0` without_seed_reasons=`{}` without_seed_details=`{}` inferred_parent_seed_id=`0` inferred_stages=`{}` ambiguous_prefix=`0` missing_parent_stages=`{}` raw_without_seed_id=`680` eligible_followup_without_seed_id=`0` raw_followup_without_seed_id=`673`
...
- conversion candidate strategy scope: scalp=`28` swing=`683` unscoped=`1`
...
- buy funnel source: present=`True` primary=`UPSTREAM_AI_THRESHOLD` matches=`['UPSTREAM_AI_THRESHOLD']` submit_drought_source_state=`not_submit_drought_critical`
...
- #9 `swing_bucket_lifecycle_flow_combo_swing_lifecycle_flow_entry_swing_entry_entry_bucket_attribution_swing_strategy_discovery_26ca74e077bc`: sample_floor -> sample_floor
- #10 `swing_bucket_lifecycle_flow_combo_swing_lifecycle_flow_entry_swing_entry_entry_bucket_attribution_swing_strategy_discovery_44479baded61`: sample_floor -> sample_floor
...
- #20 `swing_bucket_selection_discovery_arm_attribution_bottom_rebound_signal_close_retest_limit_entry_risk_capped_fixed_10_4cdc337daf57`: sample_floor -> sample_floor
