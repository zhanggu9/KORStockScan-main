# Conversion Lane - 2026-08-28

## Decision
- conversion candidates: `13`
...
- active sim policy windows: events=`166` zero_count=`0` positive_count=`166` id_without_count=`0` zero_count_effect_excluded=`False`
- active sim taxonomy contracts: pending=`4` counts=`{'canonical': 31, 'new_axis_pending_taxonomy': 4}`
- active seed candidate validation: total=`166` eligible=`166` not_match_eligible=`0` not_match_eligible_reasons=`{}` new_entry=`13` followup=`153` matched=`0` matched_true_without_seed_id=`0` unmatched=`166` new_entry_unmatched=`13` followup_unmatched=`153` eligible_without_seed_id=`0` without_seed_reasons=`{}` without_seed_details=`{}` inferred_parent_seed_id=`0` inferred_stages=`{}` ambiguous_prefix=`0` missing_parent_stages=`{}` raw_without_seed_id=`166` eligible_followup_without_seed_id=`0` raw_followup_without_seed_id=`153`
- panic scale-in no-match: events=`45` unique_sim_records=`3` missing_sim_record_id=`0` repeated_followup=`42` status_counts=`{'no_match': 45}` source_stage_counts=`{'blocked_ai_score': 45}`
- conversion candidate strategy scope: scalp=`13` swing=`0` unscoped=`0`
...
- #5 `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_63_65_source_scalp_entry_action_decision_snapshot_stale`: env_mapping -> sample_floor
...
- #8 `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_70p_source_ai_confirmed_stale_fresh_liquidity_liquidity`: lifecycle_stage_underproduction -> lifecycle_stage_underproduction
- #9 `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_63_65_source_scalp_sim_entry_ai_price_skip_order_stale_`: lifecycle_stage_underproduction -> lifecycle_stage_underproduction
...
- #11 `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_70p_source_scalp_entry_action_decision_snapshot_stale_f`: lifecycle_stage_underproduction -> lifecycle_stage_underproduction
- #12 `lifecycle_flow:combo_lifecycle_flow:entry_entry_combo_entry_spot_score_score_70p_source_scalp_entry_action_decision_snapshot_stale_s`: lifecycle_stage_underproduction -> lifecycle_stage_underproduction
